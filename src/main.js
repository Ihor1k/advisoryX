...
const serviceCards = [...servicesSlider.querySelectorAll(".services-item")];
...
function updateActiveCard() {
  ...
  const closestCard = serviceCards.reduce((closest, card) => {
    const currentDistance = Math.abs(card.offsetLeft - currentScroll);
    const closestDistance = Math.abs(closest.offsetLeft - currentScroll);
...
servicesSlider.addEventListener("pointerdown", (event) => {
  ...
  initialScrollLeft = servicesSlider.scrollLeft;

  servicesSlider.classList.add("is-dragging");
  servicesSlider.setPointerCapture(event.pointerId);
});

servicesSlider.addEventListener("pointermove", (event) => {
  if (!isDragging) return;

  const distance = event.clientX - startX;
  servicesSlider.scrollLeft = initialScrollLeft - distance;
});
...
function stopDragging(event) {
  ...
  if (servicesSlider.hasPointerCapture(event.pointerId)) {
    servicesSlider.releasePointerCapture(event.pointerId);
  }

  updateActiveCard();
}

servicesSlider.addEventListener("pointerup", stopDragging);
servicesSlider.addEventListener("pointercancel", stopDragging);

// Click a card to move it to the beginning
serviceCards.forEach((card) => {
  card.addEventListener("click", () => {
    if (servicesSlider.classList.contains("is-dragging")) return;
...
const whoSection = document.querySelector(".who-inner");
  const whoItems = document.querySelectorAll(".who-item");

  function activateWhoItem(index) {
    whoItems.forEach((item, itemIndex) => {
      const isActive = itemIndex === index;
      const button = item.querySelector(".who-trigger");
  ...
  function updateWhoAccordion() {
    // if (!whoSection || window.innerWidth <= 900) return;

    const sectionRect = whoSection.getBoundingClientRect();

    /*
      Starts changing rows shortly after the section
      reaches the upper part of the screen.
    */
    const startPoint = window.innerHeight * 0.25;
    const availableDistance =
      whoSection.offsetHeight - window.innerHeight * 0.75;

    const passedDistance = startPoint - sectionRect.top;

    const progress = Math.min(
      Math.max(passedDistance / availableDistance, 0),
      0.9999
    );

    const activeIndex = Math.floor(progress * whoItems.length);

    activateWhoItem(activeIndex);
  }

  /*
    The rows can also be opened manually by clicking.
  */
  whoItems.forEach((item, index) => {
    const button = item.querySelector(".who-trigger");

    button.addEventListener("click", () => {
      activateWhoItem(index);
    });
  });

  window.addEventListener("scroll", updateWhoAccordion, {
    passive: true
  });

  window.addEventListener("resize", updateWhoAccordion);

  updateWhoAccordion();

  const processSection = document.querySelector(".process");
  const processTrack = document.querySelector(".process-timeline-track");
  const processButtons = document.querySelectorAll(".process-step-button");
  ...
  function setProcessStep(activeIndex) {
    if (activeIndex === currentProcessStep) return;

    currentProcessStep = activeIndex;
    ...
    processContents.forEach((content, index) => {
      ...
      content.classList.toggle("is-complete", index < activeIndex);
    });
    ...
    */
    const progress =
      activeIndex / Math.max(processButtons.length - 1, 1) * 100;
  ...
  function updateProcessOnScroll() {
    ...
    const stickyTop = window.innerWidth <= 900 ? 8 : 24;
  
    const scrollDistance =
      processSection.offsetHeight - window.innerHeight;
    ...
    const scrollProgress = Math.min(
      passedDistance / Math.max(scrollDistance, 1),
      0.9999
    );
  
    const activeIndex = Math.min(
      Math.floor(scrollProgress * processButtons.length),
      processButtons.length - 1
    );
  
    setProcessStep(activeIndex);
  }
...
// const busSection = document.querySelector(".js-bus-anim");

// if (busSection) {
//   const busObserver = new IntersectionObserver(
//     (entries) => {
//       entries.forEach((entry) => {
//         if (entry.isIntersecting) {
//           busSection.classList.add("is-visible");
//           busObserver.unobserve(busSection);
//         }
//       });
//     },
//     {
...
//   busObserver.observe(busSection);
// }
